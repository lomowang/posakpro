...
function WorkCardTitle({ project }) {
  if (project.slug === "Simpleriver") {
    return (
      <>
        <span className="block whitespace-nowrap">Lakec: A Very</span>
        <span className="block whitespace-nowrap">Simple River</span>
      </>
    );
  }
  ...
  if (project.slug === "Misafafahiyan") {
    return (
      <>
        <span className="block whitespace-nowrap">Misafafahiyan</span>
        <span className="block whitespace-nowrap">Metamorphosis</span>
      </>
    );
  }
        ...
        {projects.map((project, index) => <Reveal key={project.slug} delay={index * 0.08}>
          <Link href={project.href} className="group block border-b border-black/20 pb-7">
            ...
            <div className="mt-5 flex items-start justify-between gap-5">
              <div className="min-w-0 flex-1"><p className="mb-3 text-[9px] uppercase tracking-[0.28em] text-black/50">{project.number} · {project.year}</p><h3 className={`story-display leading-[0.9] ${project.title.length > 18 ? "text-[2.05rem] sm:text-[2.35rem] md:text-[2.6vw] lg:text-[2.45rem] xl:text-[2.85rem]" : "text-3xl md:text-4xl"}`}><WorkCardTitle project={project} /></h3><p className="mt-3 text-[9px] uppercase tracking-[0.2em] text-black/50">{project.medium} · {project.duration}</p></div>
...
export function WorkDetailPage({ slug }) {
  ...
  return <main className="bg-white px-5 pb-20 text-black md:px-10 md:pb-32"><div className="mx-auto max-w-[1700px]">
    <header className="border-b border-black/20 pb-10 pt-14 md:pb-16 md:pt-24"><div className="mb-10 flex justify-between text-[10px] uppercase tracking-[0.3em] md:mb-12"><span>{project.number} / Selected work</span><span>{project.year}</span></div><div className="grid gap-8 md:grid-cols-[minmax(0,1fr)_20rem] md:items-end lg:grid-cols-[minmax(0,1fr)_22rem]"><h1 className="story-display max-w-6xl break-words text-[13vw] leading-[0.86] md:text-[6.9vw] md:leading-[0.84] xl:text-[7.2rem]">{project.title}</h1><div className="relative z-10 border-t border-black/25 bg-white pt-4 text-xs uppercase leading-7 tracking-[0.2em]"><p>Posak Jodian</p><p>{project.medium}</p><p>{project.duration}</p><p>Taiwan · {project.year}</p></div></div></header>
    <div className="relative mt-8 h-[58svh] min-h-[320px] max-h-[760px] overflow-hidden bg-black/5 md:mt-16 md:h-[72svh] md:min-h-[480px]"><Image src={project.hero} alt={project.title} fill priority className="object-cover" sizes="100vw" /></div>
    <section className="grid gap-10 border-b border-black/20 py-16 md:grid-cols-[16rem_minmax(0,1fr)] md:gap-16 md:py-28 lg:grid-cols-[22rem_minmax(0,1fr)]"><Reveal><p className="text-[10px] uppercase tracking-[0.3em]">About the work</p><p className="mt-5 text-sm leading-relaxed text-black/55">{project.year}<br />{project.medium}<br />{project.duration}</p></Reveal><Reveal delay={0.1}><div className="max-w-4xl space-y-7 text-lg leading-[1.72] md:text-2xl md:leading-[1.58] lg:text-[1.7rem]">{project.statement.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div></Reveal></section>
