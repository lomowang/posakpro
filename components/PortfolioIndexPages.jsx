...
function PageIntro({ index, title, description }) {
  return <header className="border-b border-black/25 pb-10 pt-16 md:pb-16 md:pt-24">
    <div className="mb-12 flex flex-wrap justify-between gap-4 text-[10px] uppercase tracking-[0.3em] md:mb-20"><span>Index / {index}</span><span>POSAK JODIAN</span></div>
    <div className="grid gap-8 md:grid-cols-[1fr_18rem] md:items-end lg:grid-cols-[1fr_22rem]">
      <h1 className="story-display text-[16vw] leading-[0.78] sm:text-[14vw] md:text-[9vw] lg:text-[11vw]">{title}</h1>
...
function NewsVisual({ item, priority }) {
  ...
  return (
    <div className="relative aspect-[16/10] overflow-hidden bg-black/5">
      <Image src={item.image} alt={item.title} fill priority={priority} className="object-cover transition-transform duration-700 hover:scale-[1.02]" sizes="(max-width: 768px) 100vw, 62vw" />
    </div>
  );
}
...
export function NewsIndexPage() {
  return <main className="bg-white px-5 pb-32 text-black md:px-10">
    <div className="mx-auto max-w-[1600px]">
      ...
      <div>{newsItems.map((item, index) => <Reveal key={item.title}><article className="grid max-w-full gap-8 overflow-hidden border-b border-black/20 py-14 md:py-24 xl:grid-cols-12 xl:gap-10">
        <div className={`min-w-0 max-w-full xl:col-span-7 ${index % 2 ? "xl:col-start-6" : ""}`}><NewsVisual item={item} priority={index === 0} /></div>
        <div className={`min-w-0 max-w-full flex flex-col justify-between gap-12 xl:col-span-4 ${index % 2 ? "xl:col-start-1 xl:row-start-1" : "xl:col-start-9"}`}>
          <div><div className="mb-8 flex justify-between text-[10px] uppercase tracking-[0.28em]"><span>{item.year}</span><span>0{index + 1}</span></div><h2 className="story-display break-words text-4xl leading-[0.98] md:text-5xl xl:text-6xl">{item.title}</h2><p className="mt-7 text-sm leading-relaxed text-black/65">{item.dates}<br />{item.location}</p></div>
  ...
  return <main className="bg-white px-5 pb-32 text-black md:px-10">
    <div className="mx-auto max-w-[1600px]">
      ...
      <Reveal><article className="grid gap-10 border-b border-black/20 py-14 md:grid-cols-12 md:py-24">
        <div className="relative flex min-h-[360px] max-w-full flex-col justify-between overflow-hidden border border-black/15 bg-[#f7f5ef] p-6 md:col-span-7 md:min-h-[520px] md:p-8 lg:min-h-[620px] lg:p-10">
          <div className="flex justify-between text-[10px] uppercase tracking-[0.32em] text-black/50">
            <span>C-LAB</span>
            <span>2026</span>
          </div>
          <div className="relative z-10 max-w-3xl">
            ...
            <h3 className="story-display max-w-full text-[12.5vw] leading-[0.84] text-black sm:text-[11vw] md:text-[5.8vw] lg:text-[7.2vw] xl:text-[8vw]">
              WE ARE
              <br />
              BECOMING
            </h3>
  ...
  return <main className="bg-white px-5 pb-32 text-black md:px-10">
    <div className="mx-auto max-w-[1600px]">
      ...
      <div className="py-12 md:py-20">{texts.map((item) => {
        const content = <article className={`group grid gap-6 border-t border-black/20 py-9 md:grid-cols-[5rem_minmax(0,1.3fr)_minmax(0,1fr)_10rem] md:items-start md:gap-8 md:py-12 lg:grid-cols-[5rem_minmax(0,1.3fr)_minmax(0,1fr)_12rem] lg:gap-10 ${item.href ? "cursor-pointer" : ""}`}><span className="text-[10px] tracking-[0.28em] text-black/55">{item.year}</span><div className="min-w-0"><p className="mb-3 text-[10px] uppercase tracking-[0.22em] text-black/55">{item.source}</p><h2 className="story-display text-3xl leading-none transition-opacity group-hover:opacity-55 md:text-4xl lg:text-5xl">{item.title}</h2>{item.href && <p className="mt-6 inline-block border-b border-black pb-1 text-[9px] uppercase tracking-[0.25em]">Read online ↗</p>}</div><p className="text-sm leading-relaxed text-black/65">{item.excerpt}</p>{item.image ? <div><div className="relative aspect-[4/3] overflow-hidden bg-black/5"><Image src={item.image} alt="" fill className="object-cover transition duration-500 group-hover:opacity-70" sizes="12rem" /></div>{item.href && <p className="mt-3 text-right text-[9px] uppercase tracking-[0.22em]">Open article ↗</p>}</div> : <span className="text-right text-xl">{item.href ? "↗" : "—"}</span>}</article>;
